...
- [x] ~~**Rec 18 PR #18-2:** `ItemDeserializer` hardening — declared-size precheck + running-counter cap + clean-up on failure.~~ **Shipped 2026-06-20** ([PR #467](https://github.com/CryptoJones/GayHydra/pull/467)) — constructor rejects a negative or >64 GiB (overridable via `-Dghidra.store.maxItemLength`) header-declared length; `saveItem()` copies exactly the declared budget and fails closed unless the decompressed stream matches it (symmetric with `ItemSerializer`'s `lengthWritten==length` invariant), defeating both truncated/forged files and over-producing zip-bomb payloads; the copy is wrapped so a mid-copy error/cancel disposes the stream instead of leaking it. New `ItemDeserializerTest` (6 cases) + `PackedDatabaseTest` (8) green. *(The orphaning dropped the "coordinate-disclose with NSA" gate; upstream #1481 ref informational only.)*

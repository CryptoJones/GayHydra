...
- [x] **13. Land fuzz harnesses for the C++ decompiler.** Zero public fuzz harness. 187k LOC of C++ parsing untrusted p-code/Sleigh input with 2,740 raw `new`/`delete` sites. This is the highest-EV security investment in the codebase. (Originally framed as an OSS-Fuzz integration; the [upstream submission was rejected 2026-05-26](https://github.com/google/oss-fuzz/pull/15545) — "we prefer projects with large user bases" — so harnesses run locally and via our own CI instead.) — [OSS_FUZZ.md](docs/security/OSS_FUZZ.md), [harnesses](Ghidra/Features/Decompiler/src/decompile/cpp/fuzz/)

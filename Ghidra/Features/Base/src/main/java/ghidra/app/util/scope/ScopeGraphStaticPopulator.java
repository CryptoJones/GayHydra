 ...
 *
 * <p><b>Scope of this slice.</b> Globals come from defined data units (one
 * {@link GlobalAddress} per unit); parameters come from function signatures (one
 * {@link Parameter} per slot). The RFC's third deterministic kind — "same struct field across
 * loads" — needs reference-level grounding (which loads target which field) and is deferred to its
 * own slice rather than minted speculatively from type definitions.
 *

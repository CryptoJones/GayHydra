 ...
 * {@code #37-10r}, {@link #operandExpr}); comparison operands compose the same way
 * ({@code new (buf) C((param_2 & 7) == 5)}, {@code #37-10s}). Logical negation (typically canonicalised
 * away by the decompiler) and overload resolution against the {@code DataType} signature are later
 * {@code #37-10} work.
 *
...
public final class CppPlacementConstructionDriver {
	 ...
	 * <em>comparison</em> ({@link #comparisonExpr}) such as {@code param_2 == 7} or {@code param_2 < 7}
	 ...
	 * or {@code -1} for a negative argument), so a literal argument like {@code new (buf) C(5)} renders
	 * rather than declining. A leaf is a single named variable, string literal, or constant &mdash; it
	 * carries no operator; a compound operand is rendered by {@link #binaryExpr}.
	 *
	 ...
	 * expression. (Duplicated from the heap driver as an honest per-form twin, per the DD-0026
	 ...
	 * {@code param_2 == 7} or {@code (param_2 & 7) < 5}, or null when its definition is not a
	 ...
	 * op sits one hop below it (grounded: {@code new (buf) C(v == 7)} arrives as an {@code INT_ZEXT} of an
	 * {@code INT_EQUAL} of the named {@code param_2} and the constant {@code 7}). This helper peels
	 ...
	 * ({@code new (buf) C((param_2 & 7) == 5)}, {@code #37-10s}); a cast-wrapped or unrecognised operand
	 ...
	 * &mdash; grounded, {@code ~v == 5} arrives as {@code INT_EQUAL(param_2, -6)} and renders at leaf
	 * level.)
	 *
	 ...
	 * adjusting the constant or swapping the operands, so {@code v < 7} renders {@code param_2 < 7},
	 * {@code v <= 7} renders {@code param_2 < 8}, {@code v > 7} renders {@code 7 < param_2}, and
	 * {@code v >= 7} renders {@code 6 < param_2} &mdash; each the exact boolean the p-code computes, which
	 ...
	 * the heap driver as an honest per-form twin, per the DD-0026 rule-of-three convention, until a third
	 * user earns the extraction.)
	 */
	 ...
	 * and the shifts. (Duplicated from the heap driver as an honest per-form twin, per the DD-0026
	 * rule-of-three convention, until a third user earns the extraction.)
	 */
	 ...
	 * ({@code #37-10k}, {@code #37-10l}). (Duplicated from the heap driver as an honest per-form twin, per
	 * the DD-0026 rule-of-three convention, until a third user earns the extraction.)
	 */
	 ...
	 * not-a-string signal; the empty-string narrow prefix is a successful match. (Duplicated from the
	 * heap driver as an honest per-form twin, per the DD-0026 rule-of-three convention, until a third
	 * user earns the extraction.)
	 */
	 ...
	 * the never-wrong contract. (Duplicated from the heap driver as an honest per-form twin, per the
	 * DD-0026 rule-of-three convention, until a third user earns the extraction.)
	 */

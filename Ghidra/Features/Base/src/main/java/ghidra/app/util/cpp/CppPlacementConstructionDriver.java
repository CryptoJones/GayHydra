 ...
 * such as {@code new (buf) C(param_1 + 7)} ({@code #37-10m}, {@link CppOperandRenderer}), as does a
 * single-operand unary expression &mdash; arithmetic negation or bitwise complement over a leaf such as
 * {@code new (buf) C(-param_1)} ({@code #37-10o}, {@link CppOperandRenderer}), as does a zero-extended comparison
 * over leaf operands such as {@code new (buf) C(param_1 == 7)} / {@code new (buf) C(param_1 < 7)}
 * ({@code #37-10p}, {@code #37-10q}, {@link CppOperandRenderer}). A nested compound argument renders
 * recursively with each nested level parenthesised ({@code new (buf) C((param_2 & 7) + 1)},
 * {@code #37-10r}, {@link CppOperandRenderer}); comparison operands compose the same way
...
public final class CppPlacementConstructionDriver {
	...
	private RenderedPlacement render(PcodeOp callSite, PlacementConstruction object,
		...
		String placementExpr = CppOperandRenderer.operandName(object.placementBuffer());

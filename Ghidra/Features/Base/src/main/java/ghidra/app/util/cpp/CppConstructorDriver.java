...
public final class CppConstructorDriver {
	 ...
	 * character literal ({@link #charConstantLiteral}); and an integer literal's decimal value is a
	 ...
	 * themselves {@code AbstractIntegerDataType}s; the more specific type wins. An argument that is neither
	 * named nor a boolean/char/integer constant (an unnamed non-constant temporary, or a non-integer
	 * constant) declines the whole hint ({@code null}). Rendering compound expressions and the remaining
	 * typed constants (enum names) is later {@code #37-10} work. (Duplicated from the placement driver as
	 * an honest per-form twin, per the DD-0026 rule-of-three convention, until a third user earns the
	 * extraction.)
	 *
	 ...
	 * not {@code CharDataType} and fall through to decline. (Duplicated from the placement driver as an

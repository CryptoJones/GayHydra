 ...
 * are threaded</b> ({@code #37-10b}, {@code #37-10c}, {@code #37-10d}): the constructor {@code CALL}'s inputs after the
 * call target (index 0) and the {@code this} receiver (index 1) are its explicit arguments. A named
 * argument renders as its {@link HighVariable} name and an integer-typed constant renders as its decimal
 * value (read at the varnode's byte width, so a negative argument renders {@code -1}, not a large
 * unsigned number), so {@code new ClassName(arg)} and {@code new ClassName(5)} both render; a zero-argument
 ...
 * extraction.) Rendering compound argument expressions and typed constants (chars, bools, enum names) is
 * later {@code #37-10} work, as is overload resolution against the {@code DataType} signature.
 *
...
public final class CppConstructorDriver {
	 ...
	 * for a negative argument), so a literal argument like {@code new C(5)} renders rather than declining.
	 *
	 * <p>A constant is rendered only when its {@link HighVariable} datatype is an
	 * {@link AbstractIntegerDataType}: an integer literal's decimal value is a faithful, never-wrong
	 * hint, whereas a pointer-typed constant (e.g. a global string address) rendered as a bare decimal
	 * would mislead, so it is declined. The literal is read at the varnode's own byte width
	 * ({@link #integerConstantLiteral}) &mdash; sign-extended for a signed type, full-range for an
	 * unsigned one &mdash; so a negative or wide-unsigned argument stays faithful ({@code #37-10d}). An
	 ...
	 * typed constants (chars, bools, enum names) is later {@code #37-10} work. (Duplicated from the
	 * placement driver as an honest per-form twin, per the DD-0026 rule-of-three convention, until a

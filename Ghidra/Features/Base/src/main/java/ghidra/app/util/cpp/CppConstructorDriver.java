 ...
 * (The argument helpers live in the shared {@link CppOperandRenderer}, extracted at the third user per
 * the DD-0026 rule-of-three convention.) A one-level compound argument also renders &mdash; a two-operand arithmetic, bitwise, or
 * shift expression over leaf operands such as {@code new C(param_1 + 7)} ({@code #37-10m},
 * {@link CppOperandRenderer}), as does a single-operand unary expression &mdash; arithmetic negation or bitwise
 * complement over a leaf such as {@code new C(-param_1)} ({@code #37-10o}, {@link CppOperandRenderer}), as does a
 * zero-extended comparison over leaf operands such as {@code new C(param_1 == 7)} /
 * {@code new C(param_1 < 7)} ({@code #37-10p}, {@code #37-10q}, {@link CppOperandRenderer}). A nested
 * compound argument renders recursively with each nested level parenthesised
 * ({@code new C((param_1 & 7) + 1)}, {@code #37-10r}, {@link CppOperandRenderer}); comparison operands compose

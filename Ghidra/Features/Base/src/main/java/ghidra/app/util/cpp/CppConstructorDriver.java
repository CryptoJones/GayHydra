...
public final class CppConstructorDriver {
	 ...
	 * so a literal argument like {@code new C(5)} renders rather than declining.
	 *
	 ...
	 * never-wrong), and an integer literal's decimal value is a faithful hint, whereas a pointer-typed
	 * constant (e.g. a global string address) rendered as a bare decimal would mislead, so it is declined.
	 * The integer literal is read at the varnode's own byte width ({@link #integerConstantLiteral})
	 * &mdash; sign-extended for a signed type, full-range for an unsigned one &mdash; so a negative or
	 * wide-unsigned argument stays faithful ({@code #37-10d}). An argument that is neither named nor a
	 * boolean/integer constant (an unnamed non-constant temporary, or a non-integer constant) declines the
	 * whole hint ({@code null}). Rendering compound expressions and the remaining typed constants (chars,
	 * enum names) is later {@code #37-10} work. (Duplicated from the placement driver as an honest per-form
